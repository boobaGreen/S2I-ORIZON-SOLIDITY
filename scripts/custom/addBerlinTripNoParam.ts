...
import { addDefaultTripToBerlin } from "../../utils/utils";
import fs from "fs";
...
async function main() {
  ...
  const addressesFilePath = path.join(
    __dirname,
    `iginition/deployments/chain-${chainId}`,
  ...
  // Aggiungi il viaggio predefinito a Berlino
  await addDefaultTripToBerlin(tripManager, provider);

  console.log("Default trip to Berlin added successfully.");
}
